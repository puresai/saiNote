...
{
    var A=window.open("http://" . $_SERVER['SERVER_NAME']{:U('Home/OAuth/qqLogin','',false)}/"+Math.random(),"TencentLogin","width=450,height=320,menubar=0,scrollbars=1, resizable=1,status=1,titlebar=0,toolbar=0,location=1");
}

function toWeiboLogin()
{
    var A=window.open("http://" . $_SERVER['SERVER_NAME'].{:U('Home/OAuth/weiboLogin','',false)}/"+Math.random(),"TencentLogin","width=450,height=320,menubar=0,scrollbars=1, resizable=1,status=1,titlebar=0,toolbar=0,location=1");
}
</script>
```
...
function weibo_config(){
    define( "WB_AKEY" , '@' );
    define( "WB_SKEY" , '@' );
    define( "WB_CALLBACK_URL" , 'http://" . $_SERVER['SERVER_NAME']'  . U('Home/OAuth/weiboCallback'));
}
```
...
class OAuthAction extends AppbaseAction {
    ...
    public function weiboCallback(){
        ...
        if ($token) {
            ...
            */
                echo '<script>window.close();window.opener.location.href="localhost";</script>';
            }
            //跳转到会员中心
        } else {
            echo '授权失败';
